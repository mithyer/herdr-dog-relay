...
Status: `active` for QRM-PROD-1 P4 local cross-platform validation. QRM-1 Q4/Q5 evidence and bounded package checkpoints remain preserved. P4 validates only local enrollment/allowlist/revocation, updater rollback, and disposable macOS/Linux supervision behavior around the one-process/one-UDP/one-QUIC topology; live issuance, deployment, GOAWAY/drain/restart/readiness/rebind, and P6 evidence remain unclaimed.
...
[active](1-119) 2026-08-24 | QRM-PROD-1 P4 local architecture evidence active
- Repository state: P4 activation/baseline checkpoints are Core `413c5c0`/`1c0371e`, Relay `a244ba0`/`07b1be3`, App-iOS `e58eb3f`/`fe3c62a`, and parent `52fbe34`/`484d806`; Relay governance files are being restored to versioned ownership after `0fc3563` removed them from the index.
- Validation: Relay 79 local tests, verified enrollment/revocation/no-forward isolation, updater rollback guards, and disposable macOS/Linux user-supervision fixtures pass; see `/herdr-dog/relay/docs/p4-local-validation-report`.
- Scope: preserves one Relay, one UDP listener, one device QUIC connection, and isolated session streams during local P4 validation.
...
- Residual risk: published Linux artifact/CI provenance, restart-persistent authorization consumption, and P6 lifecycle/rebind evidence remain open; the local maintenance-tick revocation closure is covered by the P4 regression.
- Next dependency: complete P4 correction review and artifact/CI evidence without activating P6.

[active](1-127) 2026-08-24 | QRM-PROD-1 P4 cross-platform architecture validation completed
- Repository state: Relay source, restored Relay-owned AGENTS/docs governance and architecture/status edits remain uncommitted; P4 remains active; Herdr, secrets, mb17 configuration and existing service state are unchanged.
- Validation: Development macOS passed 81 Relay tests with locked quality gates. Native Ubuntu 24.04 x86_64 passed 80 Relay tests, Clippy, rustfmt, rustdoc, release build, disposable archive/checksum verification and `herdogrelay --version`; Unix socket replacement and owner-only-parent fixtures pass on both host classes.
- Scope: local protected-material, allowlist, enrollment, updater, supervision and cross-platform fixture evidence around one Relay/one UDP/one QUIC topology; Linux arm64 is rejected because no supported artifact exists.
- Exclusions: no deployment, live issuance, remote update, Herdr parsing, writes, subscriptions, healthy Current, actions, passthrough or automatic retry.
- Residual risk: published Linux artifact/CI provenance, P6 live lifecycle/rebind and restart-persistent authorization evidence remain open; fresh review is still required.
- Next dependency: complete fresh P4 review and selective checkpoint validation; do not activate P6.

[active](1-135) 2026-08-24 | QRM-PROD-1 P4 post-fix architecture validation
- Repository state: Relay source and restored Relay-owned governance remain uncommitted; P4 remains active and QRM-1 architecture is unchanged.
- Validation: macOS 81-test and native Ubuntu x86_64 80-test local batteries pass; the verified two-App regression covers idle revoked active-bridge closure without heartbeat and sibling isolation.
- Scope: local protected-material, allowlist, enrollment, updater, supervision and cross-platform evidence around one Relay/one UDP/one QUIC topology.
- Exclusions: no deployment, live issuance, remote update, Herdr parsing, writes, subscriptions, healthy Current, actions, passthrough or automatic retry.
...
[active](1-143) 2026-08-24 | QRM-PROD-1 P4 post-review remediation validation
...
- Validation: macOS passed 82 Relay tests and native Ubuntu x86_64 passed 81 tests, with locked quality gates and release/archive/checksum evidence.
- Scope: local protected material, expiry-aware allowlist, enrollment/revocation, updater, supervision and one-Relay/one-UDP/one-QUIC boundary evidence.
- Exclusions: no deployment, live issuance, remote update, Herdr parsing, writes, subscriptions, healthy Current, actions, passthrough or automatic retry.
- Residual risk: published Linux CI/artifact provenance, restart-persistent authorization and P6 lifecycle/rebind evidence remain open.
- Next dependency: complete the replacement fresh review, then selectively checkpoint the approved scope; do not activate P6.

...
Status: `active` for QRM-PROD-1 P4 local cross-platform validation. P4 activation/baseline checkpoints are Core `413c5c0`/`1c0371e`, Relay `a244ba0`/`07b1be3`, App-iOS `e58eb3f`/`fe3c62a`, and parent `52fbe34`/`484d806`. The current Relay worktree restores its accidentally untracked governance sources and records only P4 local evidence requirements; it does not widen Herdr capabilities or change the opaque bridge boundary.
...
[implemented](1-285) 2026-08-24 | QRM-PROD-1 P4 Relay enrollment local slice implemented
...
- Validation: Relay 70 tests passed with locked quality gates; verified loopback evidence covers two-App admin/revocation isolation, reconnect rejection, mTLS/no-client rejection, enrollment-ALPN no-forward and enrollment quota exhaustion.
- Scope: local evidence for QRM-RLY-PROD-002/003/006/007 boundaries; updater/archive/service and broader P4 gates remain open.
- Exclusions: no live issuance/deployment claim, mb17 change, Herdr parsing, writes, subscriptions, healthy Current, actions, passthrough or automatic retry.
- Residual risk: authorization replay/expiry, lazy silent-peer revocation, updater rollback, service lifecycle and cross-architecture artifact evidence remain open.
[validated](1-290) 2026-08-24 | QRM-PROD-1 P4 local updater/service slice validated
...
- Validation: Relay 73 tests passed with locked quality gates; archive extraction/replacement rollback-copy, release-matrix, supervision-template and injection-boundary tests pass.
- Scope: local disposable updater/archive and supervision-template evidence; QRM-RLY-PROD-009 quota/enrollment evidence is covered only for the tested bounds, not all future lifecycle gates.
- Exclusions: no service installation, live issuance/deployment claim, mb17 change, Herdr parsing, writes, subscriptions, healthy Current, actions, passthrough or automatic retry.
...
[accepted](1-298) 2026-08-24 | QRM-PROD-1 P4 local enrollment/updater slice review accepted
- Repository state: Relay source/tests and local status edits remain uncommitted; P4 overall remains active; P3 status `76270bd`, Herdr, secrets and mb17 are unchanged.
...
[validated](1-306) 2026-08-24 | QRM-PROD-1 P4 local lifecycle and updater hardening validated
- Repository state: Relay source/tests and local status edits remain uncommitted; P4 remains active; P3 status `76270bd`, Herdr, secrets and mb17 are unchanged.
...
[active](1-314) 2026-08-24 | QRM-PROD-1 P4 local correction decisions active
- Repository state: P4 activation/baseline checkpoints are Core `413c5c0`/`1c0371e`, Relay `a244ba0`/`07b1be3`, App-iOS `e58eb3f`/`fe3c62a`, and parent `52fbe34`/`484d806`; Relay `0fc3563` removed tracked governance files, so this uncommitted Relay worktree restores `AGENTS.md` and `docs/` for Relay-index tracking before any P4 checkpoint.
- Validation: Relay 79 locked tests and quality gates; static template drift tests; disposable macOS LaunchAgent and Ubuntu systemd-user lifecycle fixtures; Core 315 passed/2 ignored; core-ffi 4 passed; entitled simulator 45 executed/0 failed/0 skipped.
- Scope: confirms failure-only supervision restart semantics, valid systemd start-limit placement, Relay governance ownership, and bounded local P4 evidence only.
- Exclusions: no live issuance, deployment, mb17 mutation, remote update, writes, subscriptions, healthy Current, actions, passthrough or automatic retry.
- Residual risk: published Linux artifacts/CI provenance, persisted authorization consumption, prompt revoked-peer closure, and P5/P6 live lifecycle evidence remain open.
- Next dependency: fresh correction review and remaining artifact/CI evidence; keep P4 active and do not enter P6.

[active](1-322) 2026-08-24 | QRM-PROD-1 P4 cross-platform correction validation completed
- Repository state: Relay source, restored Relay-owned AGENTS/docs governance and status edits remain uncommitted; P4 remains active; Herdr, secrets, mb17 configuration and existing service state are unchanged.
...
- Scope: protected-material, allowlist, enrollment, updater, supervision and cross-platform fixture evidence only. Linux arm64 is rejected because no supported release artifact exists.
...
- Residual risk: published Linux artifact/CI provenance, P6 lifecycle/rebind evidence and restart-persistent authorization consumption remain open; local maintenance-tick closure of idle revoked connections and active bridges is regression-tested; fresh P4 review is still required.
- Next dependency: inspect the fresh review, apply only parent-led P0-P2 corrections if needed, rerun the complete quality battery and then selectively checkpoint the approved Relay scope.

[active](1-331) 2026-08-24 | QRM-PROD-1 P4 post-fix decisions validated
- Repository state: Relay source and governance restoration are uncommitted; the restored AGENTS/docs set is staged for Relay-index tracking, while unrelated content remains excluded.
- Validation: macOS 81 tests and native Ubuntu x86_64 80 tests plus quality gates pass; active bridge revocation closure, verified allowlist admission and sibling isolation are covered.
- Scope: P4 local protected material, enrollment, allowlist/revocation, updater, supervision and cross-platform evidence.
...
- Residual risk: published Linux artifact/CI provenance, restart-persistent authorization and P6 lifecycle/rebind evidence remain open.
- Next dependency: complete final fresh review and selective Relay checkpoint validation; keep P4 active and do not enter P6.

[active](1-339) 2026-08-24 | QRM-PROD-1 P4 post-review remediation decisions validated
- Review: parent fixes close the disabled-enrollment revoke and installer-safety P1 findings plus protected-file race, rollback-verification and certificate-expiry P2 findings.
- Validation: macOS passed 82 Relay tests and native Ubuntu x86_64 passed 81 tests; locked quality, release/archive/checksum and version checks pass.
- Scope: local protected material, expiry-aware allowlist, enrollment/revocation, updater, supervision and cross-platform evidence.
- Exclusions: no mb17 deployment, live issuance, remote update, Herdr parsing, writes, subscriptions, healthy Current, actions, passthrough or automatic retry.
...
- Next dependency: replacement fresh review, then selective Core/Relay/App-iOS/parent checkpointing; keep P4 active until closure.
